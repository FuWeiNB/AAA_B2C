...
public class MailUtilsController
{
    ...
    public String sendEmail(String email)
    {
        ...
        {
            //接收者的邮箱账号，与发送的内容
            sendMail(email,"这是一封注册邮件,注册码为："+number);
    ...
    @ResponseBody
    @RequestMapping("Toe")
    public String Toe(String ma)
    {
        //判断生成的验证码是否与输入数字相等
       // System.out.println("number:"+number+"ma:"+ma);
        if(number==Integer.valueOf(ma)||(number+"").equals(ma))
        {
            return "0";//正确
        }
        else
